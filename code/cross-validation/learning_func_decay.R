...
learning_model_estimation <- function(list_route, list_link, x){
  route_file2 <- list_route[[1]]
  link_file2 <- list_link[[1]]
  route_file3 <- list_route[[2]]
  link_file3 <- list_link[[2]]
  route_file4 <- list_route[[3]]
  link_file4 <- list_link[[3]]
...
learning_model_prediction <- function(list_route, list_link, x, r2){
  route_file2 <- list_route[[1]]
  link_file2 <- list_link[[1]]
  route_file3 <- list_route[[2]]
  link_file3 <- list_link[[2]]
  ...
  n_data3 <- length(route_file3)
  n_data4 <- length(route_file4)
  #n_data2,3,4は全て同じ (同一個人の違うwaveのデータなので)
  ...
  fr_LL <- function(r2){
    ...
    for (i_data in 1:n_data3){
      ...
      #train stopping * distance * non-home dummy
      #trainstop <- dst_depth
  ...
  return(fr_LL(r2)/n_data2) #個人ごとにwave2-4の尤度を計算し，人数で割った平均尤度
}